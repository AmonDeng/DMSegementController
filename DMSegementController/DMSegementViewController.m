...
@interface DMSegementViewController ()<UIScrollViewDelegate>
...
@implementation DMSegementViewController
...
- (void)segementPageSetting {
    ...
    // this place can change Button Size
...
- (void)scrollIndicateView {
    ...
    if (_style == DMSegementStyleDefault) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat originMovedX = _style == DMSegementStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离

    CGFloat targetButtonWidth = _style == DMSegementStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == DMSegementStyleDefault? ([self widthAtIndex:[self selectedAtIndex]] - 2 * _buttonSpace) : [self widthAtIndex:[self selectedAtIndex]];
...
- (void)setStyle:(DMSegementStyle)style {
    _style = style;

    if (style == DMSegementStyleDefault) {
